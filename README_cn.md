...
无需 SSH、无需进容器，通过环境变量即可找回（支持飞牛OS、群晖、威联通等 NAS 的容器管理界面）：

1. 在容器设置中添加环境变量 `OPENLIST_ADMIN_PASSWORD`，值设为你要的新密码
2. 重启容器
3. 用 `admin` + 该密码登录

> 首次部署时设置该变量，可完全避开“初始密码只在日志打印一次”的问题。
> 注意：设置该变量后，重启会始终以变量值为准（网页里改的密码会在重启后被打回）；找回后删除该变量即可恢复网页改密。
> 本功能在 v2.2.0 及以上版本可用（此前版本变量仅对首次创建的账号生效）。
